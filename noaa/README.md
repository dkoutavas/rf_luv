...
>
> What's deployable today: just the schema and dashboard against the shared
> data layer. Bring up the always-on infra (`docker network create rf_luv_net`
> then `bash infra/up.sh`); its `ch-bootstrap` one-shot creates the `noaa`
> database and schema and the NOAA Grafana folder. The recorder will not
> actually record yet.
...
bash infra/up.sh                   # shared ClickHouse + Grafana; ch-bootstrap creates the noaa db + schema
docker exec clickhouse clickhouse-client --user noaa --password noaa_local \
...
# The shared infra stays up; nothing NOAA-specific to tear down.
```
