#!/usr/bin/env python3
"""
ClickHouse schema migration runner for the noaa pipeline.
...
Designed to run at container startup before scheduler/recorder start.
...
log = logging.getLogger("noaa-migrate")
...
def ensure_migrations_table() -> None:
    ch_query(
        """
        CREATE TABLE IF NOT EXISTS noaa.schema_migrations (
            version     String,
            name        String,
...
def get_applied_versions() -> set[str]:
    try:
        result = ch_query(
            "SELECT version FROM noaa.schema_migrations FORMAT TabSeparated"
        )
        if not result.strip():
            return set()
...
def apply_migration(version: str, name: str, path: Path) -> None:
    ...
    ch_query(
        "INSERT INTO noaa.schema_migrations FORMAT JSONEachRow",
        data=payload,
    )
