...
Location-specific seed data for `spectrum.known_frequencies`. The Athens seed
(`known_frequencies_athens.sql`) is applied **automatically** by the infra
`ch-bootstrap` one-shot when the shared data layer comes up (`bash infra/up.sh`),
*after* the migrations have created the tables. A manual load is only needed when
you write your own catalog for another location. `init.sql` and the migration
files themselves only contain schema (no INSERTs into known_frequencies after
this split).

## Why a separate directory

`init.sql` used to bake ~35 Athens-area frequencies into the schema bootstrap.
That made a fresh `git clone` + `bash infra/up.sh` produce a working pipeline,
...
## Loading a seed manually

The Athens seed loads itself via ch-bootstrap, so you normally do not run this.
To load a custom seed after the shared data layer is up (`bash infra/up.sh`,
ClickHouse healthy, migrations applied), use the shared `clickhouse` container:
...
Each seed is idempotent (`WHERE (SELECT count() FROM ...) = 0`), so it is safe to
re-run and won't duplicate rows.
